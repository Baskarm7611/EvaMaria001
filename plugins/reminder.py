...
from info import ADMINS, OWNER_ID, SUBSCRIPTION_REMINDER_MESSAGE
...
async def main_reminder_handler(client: Client, m: Message):
    all_groups = await db.filter_chat({"has_access":True, "chat_status.is_disabled":False})
    ...
    total_groups = await db.total_premium_groups_count()
    done = 0
    failed = 0
    success = 0
    ...
    async with aiofiles.open('broadcast.txt', 'w') as broadcast_log_file:
        async for group in all_groups:
            expiry_date_str, time_remaining = await db.expiry_date(group["id"])

            if time_remaining <= 172800:
                subscription_date = group['last_verified'] if group["has_access"] else None
                if not await db.is_group_verified(group["id"]):
                    await db.update_group_info(group["id"], {"has_access": False})
